...
async def test_deploy_charmhub_charm():
    async with base.CleanModel() as model:
        ...
        await model.block_until(lambda: (len(app.units) > 0 and app.units[0].machine))
...
async def test_upgrade_charm_switch_channel():
    ...
    async with base.CleanModel() as model:
        ...
        try:
            ...
            logger.warning(
                "Charm used in test_upgrade_charm_switch_channel "
        ...
        if still22:
            try:
                ...
                raise errors.JujuError(
                    "Either the upgrade has failed, or the used charm moved "
...
async def test_upgrade_local_charm():
    async with base.CleanModel() as model:
        ...
        app = await model.deploy(
            "ubuntu", series="focal", revision=15, channel="latest/stable"
        )
        await model.wait_for_idle(status="active")
        assert "ubuntu" in app.data["charm-url"]
...
async def test_upgrade_charm_resource_same_rev_no_update():
    async with base.CleanModel() as model:
        ...
        assert (
            ress["policyd-override"].fingerprint
            == ress2["policyd-override"].fingerprint
        )
...
async def test_local_refresh():
    ...
    async with base.CleanModel() as model:
        ...
        assert origin == client.CharmOrigin(
            source="local", revision=0, base=client.Base("20.04", "ubuntu")
        )
